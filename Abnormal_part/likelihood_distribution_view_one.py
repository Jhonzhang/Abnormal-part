...
def fig_one_diagram(Loglikelihood_normal, Loglikelihood_abnormal,seq_cnt,combinnation_order = 0):
 
    print("---------------" + str(seq_cnt+1) + "-----------------" ) # 第几个模型
    normal_likelihood_first = np.array(Loglikelihood_normal).tolist()
    print(type(normal_likelihood_first[0]),normal_likelihood_first[0]) 
    normal_likelihood_first_new = normal_likelihood_first[0]
    # normal_likelihood_first =  normal_likelihood_first.replace(' ','')
    # normal_likelihood_first_new = normal_likelihood_first.split(',')
    
    abnormal_likelihood_first =np.array(Loglikelihood_abnormal).tolist() 
    print(type(abnormal_likelihood_first[0]),abnormal_likelihood_first[0])
    abnormal_likelihood_first_new = abnormal_likelihood_first[0]
    # abnormal_likelihood_first =  abnormal_likelihood_first.replace(' ','')
    # abnormal_likelihood_first_new = abnormal_likelihood_first.split(',')
    
   
    normal_likelihood_first_new = [item for item in normal_likelihood_first_new if item !=''] #取出多余空格
    normal_likelihood_first_new = [float(item) for item in normal_likelihood_first_new]
    
    abnormal_likelihood_first_new = [item for item in abnormal_likelihood_first_new if item !=''] #取出多余空格
    abnormal_likelihood_first_new = [float(item) for item in abnormal_likelihood_first_new]
    ...
    this_mode_abnormal_likelihoods_deltas = []
    for each_likelihood_E in abnormal_likelihood_first_new:
        difference = abs(each_likelihood_E - mean)
        each_delta = round(difference / std_dev,3)
        this_mode_abnormal_likelihoods_deltas.append(each_delta)
    print("this mode abnormal likelihoods:",this_mode_abnormal_likelihoods_deltas)
    ...
    plt.figure()
    this_mode = 'kdeplot_one'
    fig_likelihood_path = 'fig_likelihoods' + '/' + this_mode
    if not os.path.exists(fig_likelihood_path):
        os.makedirs(fig_likelihood_path)
    noraml_likelihood_np = normal_likelihood_first_new
    abnoraml_likelihood_np = abnormal_likelihood_first_new
    # noraml_likelihood_np = this_mode_normal_likelihoods_deltas
    # abnoraml_likelihood_np = this_mode_abnormal_likelihoods_deltas
    ...
    if hist_fig_bool:
        ...
        #plt.ylim(-1,1)#仅设置y轴坐标范围
        plt.savefig(fig_likelihood_path + '/' + str(seq_cnt+1) +'_'+str(combinnation_order)+ '.png')


def fig_judge(lkh_normal,lkh_abnormal,Loglikelihood_normal, Loglikelihood_abnormal,i_th,j_th,combinnation_order = 0):
 
    print("---------------" + str(i_th+1) + '_'+ str(j_th +1) +"-----------------" ) # 第几个模型
    normal_likelihood_first = np.array(Loglikelihood_normal).tolist()
    print(type(normal_likelihood_first[0]),normal_likelihood_first[0]) 
    normal_likelihood_first_new = normal_likelihood_first[0]
    # normal_likelihood_first =  normal_likelihood_first.replace(' ','')
    ...
    difference = abs(lkh_normal - mean)
    lkh_normal_delta = round(difference / std_dev,3)
    print("this mode normal likelihoods:",lkh_normal,this_mode_normal_likelihoods_deltas,lkh_normal_delta)
    ...
    difference = abs(lkh_abnormal - mean)
    lkh_abnormal_delta = round(difference / std_dev,3)
    print("this mode abnormal likelihoods:",lkh_abnormal,this_mode_abnormal_likelihoods_deltas,lkh_abnormal_delta)
    ...
    ttest_flag = True
    if ttest_flag:
        t_statistic1, p_value1 = ttest_1samp(this_mode_normal_likelihoods_deltas, lkh_normal_delta)
        t_statistic2, p_value2 = ttest_1samp(this_mode_abnormal_likelihoods_deltas, lkh_normal_delta)

        if p_value1 < p_value2:
            closer_group = "Normal"
        else:
            closer_group = "Abnormal"

        print(f"The normal target value is closer to: {closer_group}")
        
        t_statistic1_abnormal, p_value1_abnormal = ttest_1samp(this_mode_normal_likelihoods_deltas, lkh_abnormal_delta)
        t_statistic2_abnormal, p_value2_abnormal = ttest_1samp(this_mode_abnormal_likelihoods_deltas, lkh_abnormal_delta)

        if p_value1_abnormal < p_value2_abnormal:
            closer_group = "Normal"
        else:
            closer_group = "Abnormal"

        print(f"The abnormal target value is closer to: {closer_group}")
    ...
    if frequency_fig_bool:
        ...
        #plt.ylim(-1,1)#仅设置y轴坐标范围
        plt.savefig(fig_likelihood_path + '/' + str(i_cnt+1) + '.png')
    ...
    if hist_fig_bool:
        ...
        #plt.ylim(-1,1)#仅设置y轴坐标范围
        plt.savefig(fig_likelihood_path + '/' + str(i_th+1) +'_'+str(j_th) +'_'+str(combinnation_order)+ '.png')
  

def fig_vertical_square_diagram():
