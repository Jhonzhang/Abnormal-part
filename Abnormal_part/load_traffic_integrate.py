...
def cut_common_pcap(each_common_pcap,each_time_blocks,pcap_new_store_path):
    ...
    start_each_time_blocks =formal_time_direct_output(each_time_blocks[0])
    end_each_time_blocks  = formal_time_direct_output(each_time_blocks[1])
    cmd = r'editcap -A "%s" -B "%s" %s %s' % (start_each_time_blocks, end_each_time_blocks, each_common_pcap, pcap_new_path)
    ps = Popen(cmd, stdout=PIPE, stderr=PIPE, universal_newlines=True,shell=True)
    out = ps.stdout
    return pcap_new_path
...
def cut_pcap(path_pcap):
    ...
    target_pcap_name = [target_pcap_name_list[0],target_pcap_name_list[2],target_pcap_name_list[3],
    ...
    pcap_new_store_path = '/home/ztf/Downloads/A6/pcap/pcap_new_1'# sub_pcap存储路径
    if not os.path.exists(pcap_new_store_path):
        os.makedirs(pcap_new_store_path)
    for each_pcap_name_ori in target_pcap_name:
        index_pcap = target_pcap_name.index(each_pcap_name_ori) # 获取当前的索引
        #判定是否为跨时间区间的特殊大pcap对
        if isinstance(each_pcap_name_ori,tuple):
            ...
            new_pcap_name_path = cut_special_pcap(each_special_pcap,each_special_pcap_2,time_blocks[index_pcap],pcap_new_store_path)
        else:
            # 切分正常的大pcap
            each_common_pcap = path_pcap + '/' + each_pcap_name_ori
            new_pcap_name_path = cut_common_pcap(each_common_pcap,time_blocks[index_pcap],pcap_new_store_path)

        store_time_pcap_name_dict[time_blocks[index_pcap][0]] = new_pcap_name_path # 以每个时间区间的开始时刻为key值
    print("切分好的数据包",store_time_pcap_name_dict)
    store_data(store_time_pcap_name_dict,'project_ztf/store_time_pcap_name_dict')
...
def cut_time_block_pcap():
    # 切分sub_pcap,生成meta_pcap，存储在pcap_after_dealed
    time_block_pcap_name_path_dict = read_pickle('store_time_pcap_name_dict')#存储sub_pcap路径的dict,由cut_pcap函数生成
    ...
    store_path = '/home/ztf/Downloads/A6/pcap/pcap_after_dealt' #存储meta_pcap
    ...
    for i in range(num_cores):
        ...
        cut_each_dealt_pcap(start_time_str,each_common_pcap,store_path)# 切分meta_pcap

#step 3:cal_time_features,提取特征
def cal_variance(pkt_len_list):
    ...
    return [round(np.var(first_quartile_front_array),3),round(np.var(first_quartile_back_array),3),round(np.var(second_quartile_front_array),3),
            round(np.var(second_quartile_back_array),3),round(np.var(third_quartile_front_array),3),round(np.var(third_quartile_back_array),3)]
...
def statistic_feature(pkt_len_list):
    ...
    return [mean_size_db,std_size_db,kurtosis_size_db,skew_size_db,min_db,max_db,difference_value_db,mode_db,tvar_db] + variance_db
...
def load_traffic_scattered_feature(scattered_feature_path):
    ...
    for each_file_name in all_file_names:
        ...
        print(each_file_name,"each_block 包含数据量：",len(each_data.keys()))
    ...
    for k, v in all_dict.items():
        ...
        print("guan:",k,len(this_block_data))
...
def main():
    ...
    print("打印测试结果",type(all_traffic_feature_list),len(all_traffic_feature_list),len(all_traffic_feature_list[0]))
...
if __name__ == "__main__":
    ...
    #step 2:
    # cut_time_block_pcap()#切分sub_pcap to meta_pcap,只运行一次，把提取的结果存储起来。

    #step 3：
    # cal_time_features() # 处理正常的数据块,只运行一次，把提取的结果存储起来。
