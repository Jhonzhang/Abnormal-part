...
def load_data_mul_without_traffic(load_data_tmp_name,over_load = False):
    ...
    if over_load:
        ...
        for each_block_index in range(len(data_traffic)):
            ...
            each_merge_data_pd = pd.concat([each_block_data_actor_pd, each_block_data_sensor_pd], axis=1, join='outer')
        ...
        store_data([Or,durations],load_data_tmp_name)
        return Or,durations
    else:
...
def load_data_mul_abnormal_without_traffic(load_data_tmp_name,over_load = False):
...
if __name__ == "__main__":
    ...
    all_data_pd2, durations = load_data_mul_without_traffic(load_data_tmp_name,over_load=True)
    print(durations)
    
    # 2 adnormal part
    # load_data_tmp_name_abnormal = 'load_data_abnormal_without_traffic_dat'
    # all_data_pd_abnormal, duration_abnormal,N = load_data_mul_abnormal_without_traffic(load_data_tmp_name_abnormal,over_load=True)
